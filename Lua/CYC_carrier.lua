...
function cvn.recoveryMessage()
	 ...
	 local clockTime = timer.getAbsTime() --raw seconds
	 --get the time of the start of the next minute, in seconds
	 local oneMin = clockTime + 120  --raw seconds
	 local oneMinRounded = (math.floor(oneMin/60))*60
	 local oneMinString = mist.getClockString(oneMin)
	 local oneMinStringSplit = cvn.mysplit(oneMinString, ':')


	 timer.scheduleFunction(cvn.tankstates,"", timer.getTime()+1) -- tank as follows, numbers, stand by for time hac
	 timer.scheduleFunction(cvn.timeThirty,"", timer.getTime() +(oneMinRounded-clockTime-30)) -- 30 seconds to go play 30 second time file
	 timer.scheduleFunction(cvn.playTime,oneMinStringSplit, timer.getTime() +(oneMinRounded-clockTime-28)) -- time sound i.e. 2 6 
	 
	 timer.scheduleFunction(cvn.timeFifeen,"", timer.getTime() +(oneMinRounded-clockTime-15)) -- 15 seconds to go play 30 second time file
	 timer.scheduleFunction(cvn.playTime,oneMinStringSplit, timer.getTime() +(oneMinRounded-clockTime-13)) -- time sound i.e. 2 6 

	 timer.scheduleFunction(cvn.timeMark,"", timer.getTime() +(oneMinRounded-clockTime-6)) -- MARK seconds to go plays 6 seconds early as counts down from 5,4,3 etc 
	 timer.scheduleFunction(cvn.playTime,oneMinStringSplit, timer.getTime() +(oneMinRounded-clockTime+0.5)) -- time sound i.e. 2 6 

	 -- recovery window time is the time + plus the specified extra time
	 -- we want to get this time and floor it and round it 
	 -- then we want to play the relevant sound functions on time
	 
	-- landingTime is landing time
	local landingTime =  timer.getAbsTime() + cvn.landWindowTime	
	
	local landingRounded = (math.floor(landingTime/60))*60
	local landingRoundedString = mist.getClockString(landingRounded)
	local landingRoundedStringSplit = cvn.mysplit(landingRoundedString, ':')

	
	timer.scheduleFunction(cvn.windowOpens,"", timer.getTime() + oneMinRounded-clockTime+8 )
	timer.scheduleFunction(cvn.playTime,landingRoundedStringSplit, timer.getTime()+ oneMinRounded-clockTime+9.5) -- might need timing tweaked
	
	-- --5 min warning 
	timer.scheduleFunction(cvn.windowOpens5,"", timer.getTime() + landingRounded - clockTime - 360) 
	timer.scheduleFunction(cvn.playTime,landingRoundedStringSplit, timer.getTime() + landingRounded - clockTime - 358)
	
	-- --2 min warning 
	timer.scheduleFunction(cvn.windowOpens2,"", timer.getTime() + landingRounded - clockTime - 180)
	timer.scheduleFunction(cvn.playTime,landingRoundedStringSplit, timer.getTime() + landingRounded - clockTime - 178)
	
	 --1 min warning
	timer.scheduleFunction(cvn.windowOpens1,"", timer.getTime() + landingRounded - clockTime - 120) 
	timer.scheduleFunction(cvn.playTime,landingRoundedStringSplit, timer.getTime() + landingRounded - clockTime - 118)
	
	-- --land
	timer.scheduleFunction(cvn.windowOpen,"", timer.getTime() + landingRounded - clockTime - 75) --15 seconds early 
	 


	notify("Next recovery window starts at " .. landingRoundedStringSplit[1] .. ":" .. landingRoundedStringSplit[2],120)
	notify("Tune " .. cvn.carrierFreq .. " for further information.",20)
end


function cvn.playTime(times)
	local str1 = times[2]:sub(1,1)
	local str2 = times[2]:sub(2,2)
	if str1 == "0" then
		timer.scheduleFunction(cvn.zeroRec,"", timer.getTime()+1) 
	elseif str1 == "1" then
		timer.scheduleFunction(cvn.oneRec,"", timer.getTime()+1) 
	elseif str1 == "2" then
		timer.scheduleFunction(cvn.twoRec,"", timer.getTime()+1) 
	elseif str1 == "3" then
		timer.scheduleFunction(cvn.threeRec,"", timer.getTime()+1) 
	elseif str1 == "4" then
		timer.scheduleFunction(cvn.fourRec,"", timer.getTime()+1) 
	elseif str1 == "5" then
		timer.scheduleFunction(cvn.fiveRec,"", timer.getTime()+1) 
	elseif str1 == "6" then
		timer.scheduleFunction(cvn.sixRec,"", timer.getTime()+1) 
	elseif str1 == "7" then
		timer.scheduleFunction(cvn.sevenRec,"", timer.getTime()+1) 
	elseif str1 == "8" then
		timer.scheduleFunction(cvn.eightRec,"", timer.getTime()+1) 
	elseif str1 == "9" then
		timer.scheduleFunction(cvn.nineRec,"", timer.getTime()+1) 
	end
	
	if str2 == "0" then
		timer.scheduleFunction(cvn.zeroRec,"", timer.getTime()+1.8) 
	elseif str2 == "1" then
		timer.scheduleFunction(cvn.oneRec,"", timer.getTime()+1.8) 
	elseif str2 == "2" then
		timer.scheduleFunction(cvn.twoRec,"", timer.getTime()+1.8) 
	elseif str2 == "3" then
		timer.scheduleFunction(cvn.threeRec,"", timer.getTime()+1.8) 
	elseif str2 == "4" then
		timer.scheduleFunction(cvn.fourRec,"", timer.getTime()+1.8) 
	elseif str2 == "5" then
		timer.scheduleFunction(cvn.fiveRec,"", timer.getTime()+1.8) 
	elseif str2 == "6" then
		timer.scheduleFunction(cvn.sixRec,"", timer.getTime()+1.8) 
	elseif str2 == "7" then
		timer.scheduleFunction(cvn.sevenRec,"", timer.getTime()+1.8) 
	elseif str2 == "8" then
		timer.scheduleFunction(cvn.eightRec,"", timer.getTime()+1.8) 
	elseif str2 == "9" then
		timer.scheduleFunction(cvn.nineRec,"", timer.getTime()+1.8) 
	end
